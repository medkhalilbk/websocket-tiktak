...
import { assignDeliveryManService, updateCartDone } from "./services";
const app = express();
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("companies-notifications", (obj) => {
    obj.room.forEach((item: any) => {
      console.log(item);
      ordersStatus[item.id] = {
        status: item.status,
        totalPrice: item.totalPrice,
        restaurantName: item.restaurantName,
        companiesIds: item.companiesIds,
      };
    });
    ...
    try {
      if (response === "accept") {
        ...
        if (ordersStatus[cartId]) {
          ...
          io.emit("waiting-for-delivery", { carts: ordersStatus });
          io.emit("companies-update", {
            companyIds: ordersStatus[cartId].companiesIds,
            orders: [{ id: cartId, status: "accepted" }],
            type: "order-accept",
          });
        } else {
  ...
  socket.on("product-livred", (obj: any) => {
    ...
    try {
      ...
      socket.emit("companies-update", {
        companyIds: ordersStatus[cartId].companiesIds,
        orders: [{ id: cartId, status: "done" }],
        type: "cart-done",
      });
    } catch (error) {
      console.log(error);
